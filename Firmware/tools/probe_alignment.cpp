...
int main() {
  try {
    ...
    for (double depth : {2., 5., 10., 30., 100.}) {
      ...
      const auto bore = bore_alignment(cfg.alignment, cfg.intrinsics);
      require(bore.valid, "bore geometry invalid");
      const Vec3 point = bore.sight_camera*(depth/bore.sight_camera.z);
      for (double yaw_deg : {20., 160., 300.}) for (double pitch_deg : {-10., -35., -65.}) {
        ...
        m.anchor_u_px = bore.u_norm*cfg.intrinsics.width;
        m.anchor_v_px = bore.v_norm*cfg.intrinsics.height + (.45-.22)*100;
        m.bbox_x_min_norm = (m.anchor_u_px-40)/cfg.intrinsics.width;
        m.bbox_x_max_norm = (m.anchor_u_px+40)/cfg.intrinsics.width;
        m.bbox_y_min_norm = (bore.v_norm*cfg.intrinsics.height-.22*100)/cfg.intrinsics.height;
        ...
        const Vec3 delta = target - solved*bore.origin_camera;
        const Vec3 direction = solved*bore.direction_camera;
        ...
        worst_pixel = std::max(worst_pixel, std::hypot(u-bore.u_norm*cfg.intrinsics.width,
                                                    v-bore.v_norm*cfg.intrinsics.height));
        ++count;
      }
    }
